...
const BUILD_MARK = "widget-settings-2026-02-20a";
...
function normalizeDomainsInput(allowedDomains?: string[] | null, allowedDomainsText?: string | null) {
...
export async function GET() {
  ...
  if (!widget) {
    widget = await prisma.widget.create({
      data: { tenantId, publicKey: newWidgetPublicKey(), enabled: true, allowedDomains: [] },
      select: selectWidget,
    });
