...
export async function GET(req: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ ok: false, error: "Missing or invalid key" }, { status: 400, headers: corsHeaders });
  }
  ...
  try {
    ...
    const widget = await prisma.widget.findUnique({
      ...
      select: {
        ...
        greeting: true,
        brandColor: true,
        allowedDomains: true, // for enforcement only, do not return
      },
    });

    if (!widget || widget.enabled === false) {
      return NextResponse.json({ ok: false, error: "Widget not found or disabled" }, { status: 404, headers: corsHeaders });
    }

    // ✅ Only enforce when merchant configured allowlist
    ...
    // do not leak allowedDomains
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { allowedDomains, ...publicWidget } = widget as any;
    ...
    return NextResponse.json(
      { ok: false, error: err?.code || "InternalError", message: err?.message || String(err) },
